...
public class HumeAIService : BaseFileUploadController, IHumeService
{
    ...
    public HumeAIService(ILogger<HumeAIService> logger, IHttpClientFactory httpClientFactory, IRepository repository) : base(logger)
    {
        _logger = logger;
    ...
    public async Task<IList<Record>> AudioToEmotions(string[] recordsIds, CancellationToken cancellationToken)
    {
        var records = new List<Record>();

        foreach (var recordId in recordsIds)
        {
            var record = _repository.GetByIdAsync<Record>(recordId, cancellationToken);
